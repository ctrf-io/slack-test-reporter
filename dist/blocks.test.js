...
import { createTestResultBlocks, createFailedTestBlocks, createAiTestBlocks, createMessageBlocks, createFlakyTestBlocks, createSingleAiTestBlocks, createSingleFailedTestBlocks, } from './blocks';
import { BLOCK_TYPES, TEXT_TYPES, EMOJIS, MESSAGES } from './constants';
describe('Blocks', () => {
    const mockBuildInfo = 'Build: test-build | #123 | https://example.com/build/123';
    describe('createTestResultBlocks', () => {
        it('should create blocks for passed tests', () => {
            ...
            expect(blocks[0].accessory.image_url).toContain('https://quickchart.io/chart?');
    ...
    describe('createFailedTestBlocks', () => {
        ...
        it('should create blocks for failed tests', () => {
            ...
            const totalFailedBlock = blocks.find(block => block.type === BLOCK_TYPES.SECTION &&
        ...
        it('should handle empty failed tests array', () => {
            ...
            expect(totalFailedBlock).toBeDefined();
            const buildInfoBlock = blocks.find(block => block.type === BLOCK_TYPES.SECTION &&
    ...
    describe('createMessageBlocks', () => {
        ...
        it('should include custom blocks', () => {
            ...
            const blocks = createMessageBlocks(options);
            const foundCustomBlock = blocks.find(block => block.type === BLOCK_TYPES.SECTION &&
    ...
    describe('createSingleFailedTestBlocks', () => {
        it('should create blocks for a single failed test', () => {
            ...
            const blocks = createSingleFailedTestBlocks(testName, errorMessage, mockBuildInfo);
            expect(blocks.length).toBeGreaterThan(0);
            const blockTexts = blocks
                .filter(block => block.type === BLOCK_TYPES.SECTION)
                .map(block => block.text.text)
        ...
        it('should handle undefined error message', () => {
            const testName = 'Failed Test Name';
            const blocks = createSingleFailedTestBlocks(testName, undefined, mockBuildInfo);
            expect(blocks.length).toBeGreaterThan(0);
            const blockTexts = blocks
                .filter(block => block.type === BLOCK_TYPES.SECTION)
                .map(block => block.text.text)
