 ...
 */
export function LimitFailedTests() {
    Handlebars.registerHelper('limitFailedTests', (tests, limit) => {
 ...
 */
export function moreThanHelper() {
    Handlebars.registerHelper('moreThan', (a, b) => a > b);
}
/**
 ...
 */
export function countFlakyHelper() {
    Handlebars.registerHelper('countFlaky', tests => {
        return tests.filter((test) => test.flaky).length;
    });
}
/**
 ...
 */
export function anyFlakyTestsHelper() {
    Handlebars.registerHelper('anyFlakyTests', (tests) => {
 ...
 */
export function anyFailedTestsHelper() {
    Handlebars.registerHelper('anyFailedTests', (tests) => {
 ...
 */
export function anySkippedTestsHelper() {
    Handlebars.registerHelper('anySkippedTests', (tests) => {
        return tests.some(test => test.status === 'skipped' ||
            test.status === 'pending' ||
            test.status === 'other');
    });
}
 ...
 */
export function formatDurationStartStopToHumanHelper() {
    Handlebars.registerHelper('formatDuration', (start, stop) => {
        if (start === 0 && stop === 0) {
            return 'not captured';
        }
 ...
 * @returns {string} A formatted duration string.
 */
export function formatDurationMsToHumanHelper() {
    Handlebars.registerHelper('formatDurationMs', (duration) => {
        if (isNaN(duration)) {
            return 'not captured';
        }
        if (duration < 1) {
 ...
 */
export function sortTestsByFlakyRateHelper() {
    Handlebars.registerHelper('sortTestsByFlakyRate', (tests) => {
        const testsCopy = tests.slice();
        const flakyTests = testsCopy.filter(test => test.extra !== undefined &&
            typeof test.extra.flakyRate === 'number' &&
            test.extra.flakyRate > 0);
        flakyTests.sort((a, b) => Number(b.extra?.flakyRate) - Number(a.extra?.flakyRate));
        return flakyTests;
    });
}
/**
 ...
 */
export function sortTestsByFailRateHelper() {
    Handlebars.registerHelper('sortTestsByFailRate', (tests) => {
        const testsCopy = tests.slice();
        const failedTests = testsCopy.filter(test => test.extra !== undefined &&
            typeof test.extra.failRate === 'number' &&
            test.extra.failRate > 0);
        failedTests.sort((a, b) => Number(b.extra?.failRate) - Number(a.extra?.failRate));
        return failedTests;
    });
}
/**
 ...
 */
export function formatRateHelper() {
    Handlebars.registerHelper('formatRate', (rate, fractionDigits) => {
