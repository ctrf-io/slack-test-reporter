...
import { sendSlackMessage, postMessage, updateMessage, addReaction, } from './client/index.js';
 ...
 */
async function dispatchMessage(message, options, logs, successMsg) {
    ...
    else if (options.oauthToken !== undefined &&
        options.channelId !== undefined) {
        let response;
        if (options.updateTs) {
            response = await updateMessage(options.channelId, options.updateTs, message, {
 ...
 */
async function addStatusReaction(report, ts, options) {
    if (!options.react ||
        !options.oauthToken ||
        !options.channelId ||
        !ts ||
...
export async function sendTestResultsToSlack(report, options = {}, logs = false) {
    ...
    const ts = await dispatchMessage(message, options, logs, 'Test results message sent to Slack.');
    if (ts) {
...
export async function sendFailedResultsToSlack(report, options = {}, logs = false) {
    ...
    if (options.consolidated !== undefined && options.consolidated) {
        ...
        if (message !== null) {
            const ts = await dispatchMessage(message, options, logs, 'Failed test summary sent to Slack.');
            if (ts) {
                await addStatusReaction(report, ts, options);
            }
            if (options.returnTs)
        ...
        if (!threadTs && report.results.summary.failed > 1) {
            ...
            parentTs = await dispatchMessage(summaryMsg, options, logs, 'Failure summary sent to Slack.');
            if (parentTs) {
        ...
        for (const test of report.results.tests) {
            if (test.status === 'failed') {
                ...
                if (message !== null) {
                    const ts = await dispatchMessage(message, { ...options, threadTs: parentTs || threadTs }, logs, 'Failed test summary sent to Slack.');
                    if (!firstTimestamp)
                        firstTimestamp = ts;
                }
                else {
...
export async function sendFlakyResultsToSlack(report, options = {}, logs = false) {
    ...
    if (message !== null) {
        const ts = await dispatchMessage(message, options, logs, 'Flaky tests message sent to Slack.');
        if (ts) {
            await addStatusReaction(report, ts, options);
        }
        if (options.returnTs)
...
export async function sendAISummaryToSlack(report, options = {}, logs = false) {
    if (options.consolidated !== undefined && options.consolidated) {
        ...
        if (message !== null) {
            const ts = await dispatchMessage(message, options, logs, 'AI test summary sent to Slack.');
            if (ts) {
                await addStatusReaction(report, ts, options);
            }
            if (options.returnTs)
        ...
        if (!threadTs && report.results.summary.failed > 1) {
            ...
            parentTs = await dispatchMessage(summaryMsg, options, logs, 'AI summary header sent to Slack.');
            if (parentTs) {
                await addStatusReaction(report, parentTs, options);
            }
        }
        ...
        for (const test of report.results.tests) {
            if (test.status === 'failed') {
                ...
                if (message !== null) {
                    const ts = await dispatchMessage(message, { ...options, threadTs: parentTs || threadTs }, logs, 'AI test summary sent to Slack.');
                    if (!firstTimestamp)
                        firstTimestamp = ts;
                }
                else {
...
export async function sendCustomMarkdownTemplateToSlack(report, templateContent, options = {}, logs = false) {
    ...
    if (message !== null) {
        const ts = await dispatchMessage(message, options, logs, 'Custom template message sent to Slack.');
        if (ts) {
            await addStatusReaction(report, ts, options);
        }
        if (options.returnTs)
...
export async function sendCustomBlockKitTemplateToSlack(report, templateContent, options = {}, logs = false) {
    ...
    if (message !== null) {
        const ts = await dispatchMessage(message, options, logs, 'Custom Block Kit message sent to Slack.');
        if (ts) {
            await addStatusReaction(report, ts, options);
        }
        if (options.returnTs)
