import { formatString, MESSAGES, EMOJIS, BLOCK_TYPES, TEXT_TYPES, LIMITS, NOTICES, } from './constants.js';
/**
 ...
 */
export function createTestResultBlocks(summary, buildInfo, flakyCount = 0) {
    ...
    const durationText = durationInSeconds < 1
        ...
        : formatString(MESSAGES.DURATION_FORMAT, new Date(durationInSeconds * 1000).toISOString().substring(11, 19));
...
export function createChartImage(summary) {
    const { passed, failed, skipped, pending, other, tests } = summary;
    ...
    const chartUrl = `https://quickchart.io/chart?w=150&h=150&c=${encodeURIComponent(JSON.stringify({
        type: 'doughnut',
        data: {
            datasets: [
                {
 ...
 */
export function createFailedTestBlocks(failedTests, buildInfo) {
    const blocks = [
        {
            type: BLOCK_TYPES.SECTION,
            text: {
    ...
    limitedFailedTests.forEach(test => {
        const failSummary = test.message !== undefined && test.message.length > LIMITS.CHAR_LIMIT
 ...
 */
export function createAiTestBlocks(failedTests, buildInfo) {
    const blocks = [
        {
            type: BLOCK_TYPES.SECTION,
            text: {
...
export function createMessageBlocks(options) {
    const { title, prefix = null, suffix = null, missingEnvProperties, customBlocks = [], } = options;
    const blocks = [];
    if (title !== '' && title !== null) {
        blocks.push({
            type: BLOCK_TYPES.HEADER,
 ...
 */
export function createFlakyTestBlocks(flakyTests, buildInfo) {
 ...
 */
export function createSingleAiTestBlocks(testName, aiSummary) {
 ...
 */
export function createSingleFailedTestBlocks(testName, message, buildInfo) {
    const enrichedMessage = message !== undefined && message.length > LIMITS.CHAR_LIMIT
